# Hi, my name is Vu Xuan Kien
# Contact For Work:
# Facebook: /vuxuankiendzvcl
# Zalo 0345864723
# Telegram: .comxkprj or @xkprj
# Copy me is a dog
# ██╗██╗██╗
# ██║██║██║
# ██║██║██║
...
def banner():
    text = '''
          ...
          `sMNNs-oMMMMNNNNNNNNNNNNNNNNNNMMMMo-sNNMs                                               `dM+s-:sMMMNNNNNNMMNMNNMNMMNNNNNNMMMs-:s+Md`
...
class Client(ZaloAPI):
    ...
    def onMessage(self, mid, author_id, message, message_object, thread_id, thread_type):
        try:
            ...
            logger.info(f"{message} [BY] {author_id} [IN] {thread_id}")
            ...
            if self.get_allow_link_status(thread_id) and self.is_url_in_message(message_object) and author_id not in ADMIN:
                if not self.is_group_admin(thread_id, author_id):
                  self.deleteGroupMsg(message_object.msgId, author_id, message_object.cliMsgId, thread_id)
                  self.replyMessage(Message(text=f"Admin Vũ Xuân Kiên đã xoá liên kết do {author_name} gửi trong nhóm. Vui lòng không chia sẻ liên kết trong nhóm nhé!"), message_object, author_id, ThreadType.USER)
                  return
            if message_object.msgType == 'chat.sticker':
                allowed_groups = self.load_allowed_groups()
                if thread_type == ThreadType.GROUP and thread_id in allowed_groups:
            ...
            if message and thread_type == ThreadType.USER:
                ...
                msg = f'''
~ Cung Cấp Source Chất Lượng 

⓵ Source VXK Zalo Bot
💳 : 448.000 VNĐ | Có Tiền Thì Làm Phiền Thiên Hạ
> Đôi Lời - Project Này Đầy Đủ Chức Năng, Game Giải Trí, Quản Lý Nhóm, Commands (90+) /-ok
• Được Support Tận Giường, Sẽ Không Update Nữa Nên Giá Vẫn Giữ

🔗 Group Zalo Dev&Test Bot: https://zalo.me/g/dgbqdl384

> Ưng Thì Liên Hệ Ngay:

• Zalo: 034.586.4723
• Facebook: /vuxuankiendzvcl
• Telegram: @xkprj or t.me/xkprj

🔔 Notification @@
Xin Phép Giấu Tên Con Chó ( Never Give Up )
Có Mấy Cái Api Lỏ Thì Cứ Khoe Lên Khoe Xuống, Mồm Nói Bố Dùng WormGPT Của Dương Công Bằng, Bố Show Api Gemini Ra Thì Khoe Api Check Acc Liên Quân Mobile, Check Thì Đéo Thấy Đâu Mà Acc Check Thì: Tướng 0, Level 0, Skin 0
Óc Chó Đến Thế Là Cùng Anh Em Ạ =))
'''
